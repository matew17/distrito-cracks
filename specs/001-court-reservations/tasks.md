...
- [ ] T003 [P] Document `VENUE_TIMEZONE` (default `America/Bogota`) in `.env.example` and read it via `process.env` in the time helper; BR-03's outcome must not depend on the host's `TZ` (research.md R4).
